...
public:
    DatagramPacket(const std::vector<uint8_t>& data, size_t length, in_addr_t address = INADDR_ANY, in_port_t port = 0)
        : _data(data), _length(std::min(data.size(), length)) {
    ...
    void* getData() { 
        return static_cast<void*>(const_cast<uint8_t*>(_data.data()));
    }
    ...
    void setLength(size_t length) { 
        _length = std::min(length, _data.size());
    }
